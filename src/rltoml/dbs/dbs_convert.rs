...
fn build_dbs_bin(doc: &toml_edit::Document<std::string::String>) -> Result<Vec<u8>, DbsWriteError> {
	...
	for table in column_tables.iter() {
		...
		let type_code = match type_name {
			...
			_ => return Err(DbsWriteError::InvalidStructure(format!("unknown column type '{type_name}'"))),
		};
	...
	for table in row_tables.iter() {
		...
		for (index, column) in columns.iter().enumerate() {
			...
			cells.push(match column.type_code {
				0x53 => TomlCell::String(item.as_str().ok_or_else(|| DbsWriteError::InvalidStructure(format!("'{key}' must be a string")))?.to_string()),
...
fn write_dbs_bin(columns: &[TomlColumn], rows: &[TomlRow]) -> Result<Vec<u8>, DbsWriteError> {
	...
	let string_table_offset = checked_offset(value_list_offset, row_count.checked_mul(column_count).and_then(|n| n.checked_mul(4)))?;

	let mut values = Vec::with_capacity(rows.len().saturating_mul(columns.len()));
	let mut string_table = Vec::new();
	for row in rows {
		if row.cells.len() != columns.len() {
			return Err(DbsWriteError::InvalidStructure("row column count does not match schema".into()));
		}
		for cell in &row.cells {
			match cell {
				TomlCell::String(value) => {
					let offset = u32::try_from(string_table.len()).map_err(|_| DbsWriteError::InvalidStructure("string table is too large".into()))?;
	...
	for value in [string_end, row_count, column_count, row_id_offset, type_list_offset, value_list_offset, string_table_offset] {
